...
import { updateStartedPredictionAction } from "./actions";
...
function getAlert(success?: string, error?: string) {
  ...
  const messages: Record<string, string> = {
    ...
    "match-not-editable": "Este partido no esta en juego o ya ha finalizado.",
...
function hasStarted(match: MatchRow) {
...
export default async function AdminPredictionsPage({
  ...
  await requireAuthenticatedUser({ requireAdmin: true });
  ...
  if (selectedUserId) {
    ...
    if (matchIds.length > 0) {
      ...
      const matches = ((matchesData as MatchRow[] | null) ?? []).filter(hasStarted);
    ...
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-bold text-slate-900">
          Editar prediccion de un partido iniciado
        </h1>
        <p className="mt-2 max-w-2xl text-sm text-slate-600">
          Selecciona un usuario y uno de sus partidos ya iniciados. Los partidos
          finalizados se excluyen para no modificar puntos ya concedidos.
        </p>
      </div>
      ...
      <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <form method="get">
            ...
            <label
              htmlFor="prediction-match"
              className="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500"
            >
              Partido iniciado con prediccion
            </label>
            <div className="flex gap-2">
              <select
                id="prediction-match"
              ...
              >
                ...
                {eligiblePredictions.map(({ prediction, match }) => (
                  <option key={prediction.id} value={match.id}>
                    {match.home_team} vs {match.away_team} - {formatKickoff(match.match_datetime)}
                  </option>
                ))}
              </select>
              <button
            ...
            {selectedUserId && eligiblePredictions.length === 0 && (
              <p className="mt-2 text-sm text-amber-700">
                Este usuario no tiene predicciones editables en partidos iniciados.
              </p>
            )}
          </form>
        </div>
      ...
      {selected && (
        <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                {formatKickoff(selected.match.match_datetime)} - {selected.match.status}
              </p>
            </div>
          ...
          <form action={updateStartedPredictionAction} className="mt-6">
